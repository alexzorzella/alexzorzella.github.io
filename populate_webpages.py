...
def populate_template(output_template_filename: str, output_filename: str, image_sources_directory_name: str,
                      ul_class: str | None = None, li_class: str | None = None):
    ...
    for directory in directories_to_process:
        ...
        for card_filepath in card_filepaths:
            thumbnail_path = Path(
                f'/{image_sources_directory_name}/thumbnails/{card_filepath.name.replace(".png", ".webp")}')
            asset_path = Path(
            ...
            html_image_templates.append(
                HTMLImageTemplate(thumbnail_path=thumbnail_path, asset_path=asset_path, alt=card_filepath.stem,
                                  li_class=li_class))

    render_and_write_to_template(html_image_templates=html_image_templates, output_template_path=output_template_path,
                                 output_filepath=output_filepath, ul_class=ul_class)

    print(f"Populated {output_template_path.stem} with images from {image_sources_directory_name}")


def render_and_write_to_template(html_image_templates: list[HTMLImageTemplate], output_template_path: Path,
                                 output_filepath: Path, ul_class: str | None = None):
...
if __name__ == "__main__":
    ...
    populate_template(output_template_filename="index.template.html", output_filename="index.html",
                      image_sources_directory_name="public/art", li_class="tile", ul_class="tilelist")
    populate_template(output_template_filename="metafight_cards.template.html", output_filename="cards.html",
                      image_sources_directory_name="public/metafight")
    populate_template(output_template_filename="magic_cards.template.html", output_filename="mtg.html",
